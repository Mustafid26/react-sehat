...
export default function CourseDashboard() {
  ...
  // Animasi titik loading sederhana
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#E5F7E4] to-white rounded-xl mb-[5rem]">
      {/* Header */}
      <div className="relative bg-[#88DE7C] p-4 rounded-b-3xl text-white mb-[5rem] h-[8rem]">
        {/* Selamat Datang Card (overlay) */}
        <div className="absolute top-12 left-4 right-4 bg-white text-black rounded-xl shadow-md p-4 h-[10rem] items-center flex items-center">
          <div className="content">
            <p className="text-md font-semibold mb-1">Selamat Datang, {data.name}</p>
            <p className="text-sm text-gray-500">
              Two new sections and many topics.
            </p>
            <button className="mt-3 bg-blue-100 text-blue-600 text-sm px-3 py-1 rounded-full">
              Watch Now
            </button>
          </div>
          <div className="content">
            <img
              src={doktor}
              alt="Kotak"
              className="w-32 rounded-full ml-4"
            />
          </div>
        </div>
      </div>
